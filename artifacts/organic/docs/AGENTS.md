...
workspace package (`@workspace/organic`): other artifacts depend
...
- Source character: warm Scandinavian-inspired surfaces, a restrained
  terracotta accent, Cormorant Garamond for editorial headings, and Inter for
  interface copy. Use calm, unhurried transitions; favor linen-like texture,
  subtle image reveals, and thin ornamental dividers over loud decoration.
...
`@workspace/organic/components/...`. Never use a `@/` alias inside
...
for `@workspace/organic` and renders mockups using this package's
theme and components.

## Consuming this package

Never copy token values, component source, hooks, or these docs into a consuming
artifact. Add `@workspace/organic` as a `workspace:*` dependency,
run `pnpm install`, and import directly from this package.

Read only the guides required by the current task:

- Building or styling web UI: `artifacts/organic/docs/consuming-web.md`
- Building or styling Expo UI: `artifacts/organic/docs/consuming-expo.md`
- Replacing an existing or scaffolded web theme/component library:
  `artifacts/organic/docs/migrating-web.md`
- Replacing existing or scaffolded Expo theme/hooks/components:
  `artifacts/organic/docs/migrating-expo.md`
...
- Match exact token values. Do not invent colors, fonts, spacing, or radii in a
  consuming app.
- Keep headings expressive but spare: reserve the serif face for editorial
  hierarchy and keep body copy in Inter for readability.
- Let cream, linen, taupe, and charcoal establish the page before adding a
  terracotta action or accent. Avoid cold, generic neutral treatments.
- Keep interaction motion gentle and purposeful. Image scale, underline,
  staggered reveal, and quiet elevation are appropriate; abrupt or busy motion is not.
