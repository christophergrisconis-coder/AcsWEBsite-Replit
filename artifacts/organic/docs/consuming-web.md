# Consuming Organic in web apps

Read `artifacts/organic/docs/AGENTS.md` first. This guide covers
...
`artifacts/organic/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/organic/styles.css";
```
...
`node_modules/@workspace/organic/src/components` to `content`.
...
```tsx
import { Button } from "@workspace/organic/components/ui/button";
import { cn } from "@workspace/organic/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/organic/hooks/use-toast";
```
...
`@workspace/organic/components/ui/button`. Run the app's typecheck
