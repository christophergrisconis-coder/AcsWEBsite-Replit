# Migrating Expo UI to Organic

Read `artifacts/organic/docs/AGENTS.md` and
`artifacts/organic/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/organic/lib/native-theme` and
  `@workspace/organic/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/organic/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/organic` and retains only
